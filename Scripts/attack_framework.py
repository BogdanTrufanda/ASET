...
def http_fuzzer():
    ...
    content = content.split("\n")
    for FUZZ in content:
        url = "https://juice-shop.herokuapp.com/" + FUZZ.strip(" ")
        r = requests.get(url)
...
def create_initial_gui():
    sg.theme('Reddit')
    layout = [[sg.Button('Brute forcer',size=(10, 2)), sg.Button('HTTP Fuzzer',size=(10, 2))], [sg.Button('Password\ncracker',size=(10, 2)),sg.Button('OSINT',size=(10, 2))],[sg.Button('Forged JWT',size=(10, 2)), sg.Button('Vulnerable\ncomponents',size=(10, 2))],[sg.Button('Exit',size=(10, 2))]]
    window = sg.Window('Attack framework', layout)
    while True:
        event, values = window.read()
        if event == sg.WIN_CLOSED or event == 'Exit':
            break
        if event == sg.WIN_CLOSED or event == 'Brute forcer':
            gui_funct()
        if event == sg.WIN_CLOSED or event == 'HTTP Fuzzer':
            threading.Thread(target=http_fuzzer, daemon=True).start()
    window.close()
